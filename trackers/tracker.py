...
class Tracker:
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        tracks = {
            'players': [],  # {0:{"bounding_box":[x1,y1,x2,y2]},11:{"bounding_box":[x1,y1,x2,y2]},
                            #  1:{"bounding_box":[x1,y1,x2,y2]},11:{"bounding_box":[x1,y1,x2,y2]}...}
                            # }
            'referees': [],
            'ball': []
        }
        ...
        for frame_num, detection in enumerate(detections):
            ...
            cls_names_inv = {v:k for k,v in cls_names.items()} # {value:key,v1:k1,v2:k2}
            
            #Supervision detection format Conversion is [frame_num, x1, y1, x2, y2, class_id, confidence]
    ...
    # Fills in the missing ball positions in a sequence of video frames
    ...
    def draw_team_ball_control(self,frame,frame_num,team_ball_control):
        ...
        cv2.putText(frame,f"Team 1 Ball Control: {team_1*100:.2f}%",(1400,900),cv2.FONT_HERSHEY_SCRIPT_SIMPLEX,1,(0,0,0),3)
        cv2.putText(frame,f"Team 2 Ball Control: {team_2*100:.2f}%",(1400,950),cv2.FONT_HERSHEY_SCRIPT_SIMPLEX,1,(0,0,0),3)
