...
class CameraMovementEstimator():
    ...
    def get_camera_movement(self,frames,read_from_stub=False,stub_path=None):
        ...
        old_features = cv.goodFeaturesToTrack(old_gray,**self.features) # expantion of **self.features
    ...
    def estimate_camera_movement(self, image1, image2):
    ...
    def draw_camera_movement(self, frames, camera_movement_per_frame):
        ...
        for frame_num, frame in enumerate(frames):
            ...
            overlay = frame.copy()
            cv.rectangle(overlay, (0,0), (500,100), (222,222,222), -1)
            alpha = 0.69
            cv.addWeighted(overlay, alpha, frame, 1-alpha, 0, frame)
            
            x_movement, y_movement = camera_movement_per_frame[frame_num]
            frame = cv.putText(frame, 'Camera Movement Estimation', (10,30), cv.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv.LINE_AA)
            frame = cv.putText(frame, f'X: {x_movement:.2f} Y: {y_movement:.2f}', (10,60), cv.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv.LINE_AA)
